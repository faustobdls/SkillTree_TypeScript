...
import {
  BaseSkillTreeRenderer,
  RenderLayer,
  IHighlight,
  ISpriteSheetAsset,
  IConnnection,
} from './BaseSkillTreeRenderer';
...
export class PIXISkillTreeRenderer extends BaseSkillTreeRenderer {
  ...
  constructor(
    ...
    this.pixi = new PIXI.Application({
      ...
      sharedTicker: true,
      backgroundColor:
        skillTreeData.patch.compare(versions.v3_16_0) >= 0
          ? 0x070b10
          : 0x1a1411,
    });
    ...
    const zoomPercent =
      this.skillTreeData.imageZoomLevels.length > 2
        ? this.skillTreeData.imageZoomLevels[1] -
          this.skillTreeData.imageZoomLevels[0]
        : 0.1;

    // fausto
    // skillTreeData

    // const nodes = skillTreeData.getNodes(SkillNodeStates.None);
    const nodes = skillTreeData.getNodes(SkillNodeStates.Active);
    // const nodes = skillTreeData.getNodes(SkillNodeStates.Hovered);
    // const nodes = skillTreeData.getNodes(SkillNodeStates.Pathing);
    // const nodes = skillTreeData.getNodes(SkillNodeStates.Highlighted);
    // const nodesX = skillTreeDataCompare?.getNodes(SkillNodeStates.Active);

    let downPointX: number = 0;
    let downPointY: number = 0;
    let upperPointX: number = 0;
    let upperPointY: number = 0;

    window.SkillTreeData = window.SkillTreeData || skillTreeData;

    // console.log(nodes);
    // console.log(skillTreeData.nodesInState);
    // console.log(skillTreeData.nodesInState[1]);
    // console.log(skillTreeData.nodesInState[SkillNodeStates.Active]);
    for (let element1 in skillTreeData.nodesInState[1]) {
    ...
    for (let id in nodes) {
      ...
      if (node.x > upperPointX || node.y > upperPointY) {
        upperPointX = node.x;
        upperPointY = node.y;
      }
      if (node.x < downPointX || node.y < downPointY) {
        downPointX = node.x;
        downPointY = node.y;
      }
    }
    // console.log({ x: upperPointX, y: upperPointY });
    // console.log({ x: downPointX, y: downPointY });
    ...
    if (skillTreeData.tree === 'Atlas') {
      ...
      let centerX = upperPointX - downPointX;
      let centerY = upperPointY - downPointY;

      let moveCenterX = centerX * (zoomPercent / 2) - 2000;
      let moveCenterY = centerY * (zoomPercent / 2) - 2000;
      // console.log(centerX);
      // console.log(centerY);
      // console.log(moveCenterX);
      // console.log(moveCenterY);
      this.viewport.moveCenter(moveCenterX, moveCenterY);
    }
    this.viewport.fitWorld(true);
    this.viewport.zoomPercent(1.726);

    this.viewport.on('pointerdown', (event) =>
      SkillTreeEvents.viewport.fire('down', this.viewport.toWorld(event.global))
    );
    this.viewport.on('pointermove', (event) =>
      SkillTreeEvents.viewport.fire('move', this.viewport.toWorld(event.global))
    );
    this.viewport.on('pointerup', (event) =>
      SkillTreeEvents.viewport.fire('up', this.viewport.toWorld(event.global))
    );
    this.viewport.on('pointercancel', () =>
      SkillTreeEvents.viewport.fire('cancel')
    );
  ...
  public PositionedOn(): void {
    const startNonAscendancyNodesEntries = Object.entries(
      this.skillTreeData.classStartNodes
    ).map(([_, node]) => _);

    const activeNonAscendancyNodesEntries = Object.entries(
      this.skillTreeData.getNodes(SkillNodeStates.Active)
    );
    const activeNonAscendancyNodeArray = activeNonAscendancyNodesEntries.filter(
      ([_, node]) =>
        node.ascendancyName === '' && startNonAscendancyNodesEntries.includes(_)
    );
    if (activeNonAscendancyNodeArray[0][1]) {
      const activeNonAscendancyNode = activeNonAscendancyNodeArray[0][1];
      this.viewport.moveCenter(
        activeNonAscendancyNode.x,
        activeNonAscendancyNode.y
      );
    }
  }
  ...
  private LoadAssets = (
    ...
    if (filteredData.length <= 0) {
      throw new Error(
        'SkillTreeData has not been defined. Could not load assets.'
      );
    }
      ...
      for (const id in dict) {
        ...
        if (filename === 'LOAD_COORDS') {
          for (const coord in sheet.coords) {
            ...
            const spritesheetData = this.GetCoordSpritesheetData(
              sheet,
              key,
              coord,
              texture
            );
            this.NodeSpritesheets[key]?.push(
              new PIXI.Spritesheet(texture.baseTexture, spritesheetData)
            );
          }
        } else {
    ...
    for (const key in this.NodeSpritesheets) {
      ...
      if (sheets !== undefined) {
        for (const i in sheets) {
          const promise = new Promise<boolean>((resolve) =>
            this.NodeSpritesheets[key]![i].parse().then((_) => resolve(true))
          );
  ...
  private GetCoordSpritesheetData = (
    spriteSheet: ISpriteSheet,
    key: string,
    id: string,
  ...
  private GetSpritesheetFrameKey = (key: string, id: string): string => {
    return `${key}/${
      id.startsWith('PassiveSkillScreen')
        ? id.replace('PassiveSkillScreen', '')
        : id
    }`;
  };
  protected DrawSpriteSheetAsset = (
    layer: RenderLayer,
    asset: ISpriteSheetAsset
  ): { width: number; height: number } => {
    return this.DrawSpriteSheetAssets(layer, [asset])[0];
  };

  protected DrawSpriteSheetAssets = (
    layer: RenderLayer,
    assets: ISpriteSheetAsset[]
  ): { width: number; height: number }[] => {
    const sizes: { width: number; height: number }[] = [];
    const container = this.GetLayer(layer);

    for (var asset of assets) {
      const texture = this.GetSpritesheetTexture(
        asset.patch,
        asset.key,
        asset.icon
      );
  ...
  private DrawArcConnection = (
    ...
    mask.arc(
      node.nodeGroup.x * node.scale,
      node.nodeGroup.y * node.scale,
      node.orbitRadii[node.orbit] * node.scale,
      startAngle,
      endAngle,
      false
    );
  ...
  private DrawLineConnection = (
    connection: IConnnection
  ): PIXI.Container | null => {
    const node =
      connection.node.skill > connection.other.skill
        ? connection.node
        : connection.other;
    const other =
      connection.node.skill > connection.other.skill
  ...
  public CreateScreenshot = async (
    mimeType: 'image/jpeg' | 'image/webp'
  ): Promise<string> => {
    return await (this.pixi.renderer as any).extract.base64(
      this.viewport,
      mimeType,
      1
    );
  ...
  protected RenderTooltip = (hovered: SkillNode): void => {
    ...
    if (tooltip !== undefined && hovered !== undefined) {
      ...
      if (tooltipCompare !== undefined && hoveredCompareNode !== undefined) {
        ...
        tooltipCompare.scale.set(
          tooltipCompare.width / boundsCompare.width / devicePixelRatio,
          tooltipCompare.height / boundsCompare.height / devicePixelRatio
        );
      }
    }
  ...
  private CreateTooltip = (node: SkillNode) => {
    let tooltip: PIXI.Container | undefined =
      this.NodeTooltips[`${node.GetId()}_${node.patch}`];
    ...
    if (tooltip === undefined) {
      ...
      let flavour: PIXI.Text | null =
        node.flavourText.filter(utils.NotNullOrWhiteSpace).length > 0
          ? new PIXI.Text(
              `\n${node.flavourText
                .filter(utils.NotNullOrWhiteSpace)
                .join('\n')}`,
